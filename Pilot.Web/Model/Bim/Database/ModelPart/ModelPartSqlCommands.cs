...
namespace Pilot.Web.Model.Bim.Database.ModelPart
{
    ...
    {
        private readonly SQLiteParameter _paramParentId;
        private readonly SQLiteParameter _paramName;
        private readonly SQLiteParameter _paramType;
        private readonly SQLiteParameter _paramPlacement;
        private readonly SQLiteParameter _paramData;

        public NodeUpdater(SQLiteConnection connection) : base(connection)
        {
            _command = new SQLiteCommand(connection)
            {
                CommandText = "INSERT INTO [nodes] (object_id, revision, version_id, parent_id, name, type, data, placement) VALUES (@object_id, @revision, @version_id, @parent_id, @name, @type, @data, @placement)"
            };

            _paramParentId = new SQLiteParameter { DbType = DbType.Guid, ParameterName = "@parent_id" };
            _paramName = new SQLiteParameter { DbType = DbType.String, ParameterName = "@name" };
            _paramType = new SQLiteParameter { DbType = DbType.String, ParameterName = "@type" };
            _paramPlacement = new SQLiteParameter { DbType = DbType.Binary, ParameterName = "@placement" };
            _paramData = new SQLiteParameter { DbType = DbType.String, ParameterName = "@data" };

            _command.Parameters.Add(_paramData);
            _command.Parameters.Add(_paramParentId);
            _command.Parameters.Add(_paramName);
            _command.Parameters.Add(_paramType);
            _command.Parameters.Add(_paramPlacement);
        ...
        public void Insert(IfcNode node, Guid versionId, DateTime revision)
        {
            ...
            _paramRevision.Value = revision.ToString(ModelPartSqlConstants.DATE_FORMAT);
            _paramVersionId.Value = versionId;

            _paramParentId.Value = node.ParentGuid;
            _paramName.Value = node.Name;
            _paramType.Value = node.Type;
            _paramData.Value = node.Attributes;
            _paramPlacement.Value = MeshPropertiesSerializer.Serialize(node.MeshesProperties);
        ...
        public void InsertDeleted(Guid node, Guid versionId, DateTime revision)
        {
            ...
            _paramParentId.Value = null;
            _paramName.Value = null;
            _paramType.Value = null;
            _paramData.Value = null;
            _paramPlacement.Value = null;
    ...
    class TessellationUpdater : IDisposable
    {
        ...
        public TessellationUpdater(SQLiteConnection connection)
        {
            ...
            _paramRevision = new SQLiteParameter { DbType = DbType.String };
        ...
        public void InsertOrUpdate(Guid id, ModelMesh tessellation, Guid objectId, Guid versionId, DateTime revision)
        {
            _paramId.Value = id;
            _paramObjectId.Value = objectId;
            _paramRevision.Value = revision.ToString(ModelPartSqlConstants.DATE_FORMAT);
            _paramData.Value = ModelMeshSerializer.Serialize(tessellation);
            _command.ExecuteNonQuery();
        }
    ...
    class BaseUpdater : IDisposable
    {
        protected SQLiteCommand _command;

        protected readonly SQLiteParameter _paramId;
        protected readonly SQLiteParameter _paramRevision;
        protected readonly SQLiteParameter _paramVersionId;

        public BaseUpdater(SQLiteConnection connection)
        {
            _paramId = new SQLiteParameter { DbType = DbType.Guid, ParameterName = "@object_id" };
            _paramRevision = new SQLiteParameter { DbType = DbType.String, ParameterName = "@revision" };
            _paramVersionId = new SQLiteParameter { DbType = DbType.Guid, ParameterName = "@version_id" };
        ...
        protected void ConstructCommand()
        {
            _command.Parameters.Add(_paramId);
            _command.Parameters.Add(_paramRevision);
            _command.Parameters.Add(_paramVersionId);
        }
