...
export class SearchApi {
    ...
    searchObjectsAsync(request: string, isContextSearch: boolean, contextObjectId: string, cancel: Subject<any>): Promise<IObject[]> {
        return new Promise((resolve, reject) => {
            const headers = this.headersProvider.getNewHeaders();
            const url = `api/Search/SearchObjects?searchRequest=${request}&isContextSearch=${isContextSearch}&contextObjectId=${contextObjectId}`;
            this.http
